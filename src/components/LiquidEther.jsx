...
export default function LiquidEtherBackground({
  ...
  useEffect(() => {
    ...
    const paletteTex = makePaletteTexture(colors && colors.length ? colors : ['#22d3ee','#10b981','#0ea5e9']);
...
`;
  ...
  return (
    <div
      ref={mountRef}
      className={`w-full h-full fixed inset-0 -z-10 overflow-hidden pointer-events-none touch-none ${className || ''}`}
      style={style}
    />
  );
}
